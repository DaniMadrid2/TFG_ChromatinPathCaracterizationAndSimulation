#version 300 es
//? Gradient descent refinement with L1+L2 regularization over the packed coefficient vector [f terms..., s terms...].
//* Purpose: stabilize coefficients and reduce overfit after AFP0 while keeping sparsity.
...
//!Salida
//? Add L2 penalty, optimize by descent, then keep the better of init vs optimized.
//! Writes to RGBA targets sized [tauMax, tauMax].
layout(location = 0) out vec4 tauXiFFinal; //! packed coeffs[0..3]
layout(location = 1) out vec4 tauXiSFinal; //! packed coeffs[4..7]
layout(location = 2) out vec4 tauXiMetaFinal; //! [cost, valid(0/1), nUsed, reserved]
...
float tauSObsErr(float aKM, float aErr){
    ...
    //Revisor: corregido el comentario para dejar constancia de que la penalizacion L2 no arregla un peso mal definido por si sola.
    //Revisor: por eso se mantiene aqui la misma propagacion rigurosa de error que en LS y AFP0.
...
void main(){
    ...
    for (int it=0; it<24; it++){
        ...
        for(int i=0; i<TAU_MAX_TOTAL_TERMS; i++){
            ...
            float l2 = (i < TAU_F_TERMS) ? l2F : l2S;
    ...
    float outCost = keepInit ? jB : jA;
    float outN = keepInit ? nB : nA;
    float outValid = (outCost < 1e8 && outN >= float(max(TAU_F_TERMS, TAU_S_TERMS))) ? 1.0 : 0.0;

    tauPack(outCoeffs, tauXiFFinal, tauXiSFinal);
    tauXiMetaFinal = vec4(outCost, outValid, outN, 0.0);
}
