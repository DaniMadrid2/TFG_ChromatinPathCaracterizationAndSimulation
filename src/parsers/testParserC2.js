...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const probePath = path.join(repoStyleDir, "parseTextC2.shaderdsl.ts");
        ...
        const parseTextPath = path.join(baseDir, "parseTextC2.shaderdsl.ts");
        const outPath = path.join(baseDir, "generatedParserC2.ts");
    ...
    const __initTauControls=()=>{
        ...
        __tauCtlTauMin=mkNum("58px","1","1","100");
        const lbMax=document.createElement("span"); lbMax.textContent="tauMax";
        __tauCtlTauMax=mkNum("58px","100","1","100");
        ...
        __tauCtlRoot.append(lbC,__tauCtlChrom,lbMin,__tauCtlTauMin,lbMax,__tauCtlTauMax,bt);
        ...
        __tauCtlRoot.append(sep,line,lbT,__tauCtlBestTau,lbS,__tauCtlBestSub,__tauCtlBestBtn,sep2,lbRank,__tauCtlRank,lbRankMode,__tauCtlRankMode,__tauCtlRankBtn,__tauCtlCopyBtn);

        // Inserta SIEMPRE en la columna izquierda de F2.
        let inserted=false;
        ...
        if(useMeanMode && tauSel>1){
            ...
            if(meanCnt>0){
                ...
                for(let i=1;i<n;i++){
                    ...
                    simX[i]=__safeNum(simX[i-1]+stepDet+stepSto,simX[i-1]);
                }
            }
        }
...
`;
        if (outCode.includes("__applyCanvasFullscreenC2")) {
        ...
        try {
            ...
            const fsSnippetJs = fsSnippet
                .replace(/ as any/g, "")
                .replace(/const __fsTargetC2=canvas;/, "const __fsTargetC2=canvas;");
            if (outJs.includes("__applyCanvasFullscreenC2")) {
        ...
        console.log("[DetailedParser-C2] generado:", outPath);
    });
}
main().catch((err) => {
    console.error("[DetailedParser-C2] error al transpilar:", err);
    process.exit(1);
});
export {};
