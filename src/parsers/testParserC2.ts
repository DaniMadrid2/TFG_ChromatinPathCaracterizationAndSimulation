...
async function main() {
    ...
    const baseDir = await resolveBaseDir("parseTextC2.shaderdsl.ts");
...
async function loadPreTemplate(baseDir: string, snippetId: string) {
    ...
    throw new Error(`No se encuentra 00_pre_imports_and_async_func_wrapper.snippet.ts para ${snippetId}`);
}

async function buildPreSnippet(baseDir: string, snippetBody: string) {
    const template = resolveAxisSnippet(await loadPreTemplate(baseDir, "c2"), 0);
    return template.replace("__SNIPPET_BODY_C2__", snippetBody);
}
