import * as pako from "/ExternalCode/mat4js/pako.esm.mjs"

export async function read(data) {
	async function readDataElem(data, index) {
		...
		if (longFormatFlag) {
			...
			if(length<0)
				length=view.getInt16(index + 4, !en)
			taglength = 4
		}
		// console.log("reading nBytes",length)
		...
		case 14: // matrix
			...
			// Array dimensions
			var dim = await readDataElem(data, reader)
			reader += dim.length
			...
			// Array name
			var name = ""
			var nameArr = await readDataElem(data, reader)
			...
			case 1: // Cell array
				...
				for (var i = 0; i < elemNum; i++) {
					const subArr = await readDataElem(data, reader)
			...
			case 2: // Structure
				arrData = {}
				var nameLength = await readDataElem(data, reader)
				reader += nameLength.length
				var fieldNameFlat = await readDataElem(data, reader)
				...
				if (dim.data[0] != 1 || dim.data.length > 1) {
					...
					for (var i = 0; i < elemNum; i++) {
						...
						for (var j = 0; j < fieldNames.length; j++) {
							var field = await readDataElem(data, reader)
							reader += field.length
			...
			case 4: // Character array
				flat = await readDataElem(data, reader)
				reader += flat.length
				...
				break
			case 5: // Sparse array
				var ir = await readDataElem(data, reader)
				reader += ir.length
				var jc = await readDataElem(data, reader)
				reader += jc.length
				var pr = await readDataElem(data, reader)
				reader += pr.length
				var numArr
				if (realFlag) {
					var pi = await readDataElem(data, reader)
					reader += pi.length
					numArr = []
			...
			case 15: // 64-bit, unsigned integer
				...
				if (realFlag) {
					...
					pr = await readDataElem(data, reader)
					reader += pr.length
					flat = pr.data
				}
				arrData = iterateN(dim.data, flat)
			...
			return read
		case 15: // compressed
			const compressed = new Uint8Array(data.slice(index + 8, index + 8 + length));
			// console.log(compressed.slice(0, 4)); // [120, 156, ...] -> 0x78 0x9C
			// console.log("compressed bytes", [...compressed.slice(0, 8)]
  			// 	.map(b => b.toString(16).padStart(2, "0")).join(" "));
			const inflated = await decompressZlib(compressed);
			
			const plain = await readDataElem(inflated.buffer, 0, en);
			return plain;
	...
	while (index < data.byteLength) {
		// console.log("index",index,"dataLength",data.byteLength);
		const elem = await readDataElem(data, index)
...
async function decompressZlib(data) {
  ...
  // Intenta primero con el encabezado completo (RFC 1950)
  try {
    ...
    console.warn("Fallback: removing zlib header/checksum", err);
    // Si falla, quita encabezado (2 bytes) + checksum (4 bytes)
    const deflateData = data.subarray(2, data.length - 4);
    const blob = new Blob([deflateData]);
