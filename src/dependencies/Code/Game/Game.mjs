...
export function createCanvas(id = "c1", w = 1080, h = 720, sc, contexttype = "2d") {
    ...
    let collec = {
        ...
        modern: (layer = new Layer(canvas.id, collec.canvas, sc ?? new Scene()), autoadd = true) => {
...
function viewPortHeight() { var de = document.documentElement; if (!!window?.innerWidth) {
    return window?.innerHeight;
}
else if (de && !isNaN(de.clientHeight)) {
    return de.clientHeight;
} return 0; }
function scrollY() { if (window?.scrollY !== undefined) {
...
export function ModernCanvas(canvas, options = {}) {
    ...
    (canvas.boundingBox) = { ...options }?.boundingBox || {
        x: 0,
        y: 0,
        w: window?.innerWidth,
        h: window?.innerHeight,
    };
    if ((typeof options?.boundingBox?.w == "function" && options?.boundingBox?.w?.()) || options?.boundingBox?.w)
        (canvas.boundingBox).w = options.boundingBox.w;
    if ((typeof options?.boundingBox?.h == "function" && options?.boundingBox?.h?.()) || options?.boundingBox?.h)
...
export class ModernCtx {
    ...
    rRect = (x = 0, y = 0, w, h, color = "black", rot = 0) => {
        h ??= this.layer.getHeight();
        w ??= this.layer.getWidth();
        const W = this.ctx.canvas.width;
        const H = this.ctx.canvas.height;
    ...
    rImg = (img, x, y, w = img.width, h = img.height, imgsc = 1, deg = 0, flipX = false, flipY = false) => {
        if (!img || !img.loaded)
            return;
        w ??= img.width ?? 100;
        h ??= img.height ?? 100;
...
export class GameIOStats {
    static getProps(stats) {
        ...
        if (typeof (stats) == "function" && !!stats.prototype) {
            return GameIOStats.getPropsFromObject(stats?.IOstats);
        }
        return undefined;
    }
...
export class CollisionManager {
    ...
    handleCollision(obj1, obj2, dt) {
        ...
        if (this.callObjects) {
            obj1?.onCollision(obj2, closestPt);
            obj2?.onCollision(obj1, closestPt);
        }
    }
}
export class ImgLoader {
    ...
    static async fromCanvasOrBitmap(source, w, h) {
        let blob;
        if (source instanceof HTMLCanvasElement) {
            blob = await new Promise((res) => source.toBlob(res));
        }
            ...
            blob = await new Promise((res) => tmp.toBlob(res));
        }
        else {
    ...
    imgs.forEach((u) => {
        if (Array.isArray(u) && isNaN(u[0])) {
            ...
            u.forEach((ur) => {
                if (!ur)
                    return;
                let timg = ImgLoader.cut(img, ur.x ?? ur[0], ur.y ?? ur[1], ur.w ?? ur[2], ur.h ?? ur[3]);
                if (timg)
                    timgs.push(timg);
            });
        }
        else {
            let timg = ImgLoader.cut(img, u.x ?? u[0], u.y ?? u[1], u.w ?? u[2], u.h ?? u[3]);
            if (timg)
                timgs.push(timg);
        }
    });
...
export class Layer extends ObjectManager {
    ...
    draw(...args) {
        ...
        this?.preExtraDraw?.(this.canvas.ctx, ...args);
        this.func("draw", this.canvas.ctx, ...args);
        this?.posExtraDraw?.(this.canvas.ctx, ...args);
    }
    tick(...args) {
        this.canvas.ctx;
    ...
    killObjs(filterfc = (o) => !!o && o?.dead !== true) {
