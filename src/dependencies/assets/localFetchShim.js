    ...
    try {
        ...
        if (at >= 0) {
            ...
            if (localPath.startsWith('/src/'))
                set.add(localPath.slice(4));
        }
        if (pathname.endsWith('/alive_2.mat'))
            set.add('/data/alive_2.mat');
    }
    catch { }
    return [...set];
}
function rewriteAssetUrl(raw) {
    if (raw.startsWith('/Imgs/'))
        return new URL('../static/Images/' + raw.slice('/Imgs/'.length), import.meta.url).toString();
    return raw;
}
export function installLocalFileShims() {
    ...
    if (desc?.set && desc.get) {
        Object.defineProperty(HTMLImageElement.prototype, 'src', {
            ...
            set(value) { return desc.set.call(this, rewriteAssetUrl(String(value ?? ''))); }
        });
    }
    const patchExistingImages = () => {
        document.querySelectorAll('img[src^="/Imgs/"]').forEach((img) => {
            ...
            if (src)
                img.setAttribute('src', rewriteAssetUrl(src));
        });
    };
    if (document.readyState === 'loading')
        document.addEventListener('DOMContentLoaded', patchExistingImages, { once: true });
    else
        patchExistingImages();
}
