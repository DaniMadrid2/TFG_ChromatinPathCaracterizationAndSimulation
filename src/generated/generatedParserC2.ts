    ...
    while(tauAFPOptPass<tauAFPOptMaxPasses && !tauAFPOptDone){
        ...
        while(tauNMInner<tauAFPOptChunk){
            ...
            while(tauAdjX0<tauMaxVeces){
                ...
                tauArnoldiCoeff.use?.();
                tauArnoldiCoeff.uNum("tauArnoldiReorth", false, false).set((Math.max(1, Math.min(3, ~~tauArnoldiReorth))));
                ...
                lastUsedProgram?.use?.();
                lastUsedProgram?.setViewport(...[0,0,9,tauAdjCount*tauMaxVeces*9]);
                lastUsedProgram?.drawArrays("TRIANGLES", 0, 6);
                await tauAdjExp.use?.();
                lastUsedProgram = tauAdjExp;
                tauAdjExp.use?.();
