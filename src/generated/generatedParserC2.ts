...
(async ()=>{
    ...
    let {screen:screen_props}=createCanvasNextTo(undefined,"c2","#F2",
...
*/
    ...
    const __c2Host=((canvas.parentElement as HTMLElement)||canvas as any) as HTMLElement;
    ...
    const __publishPanelBest=()=>{
        (window as any).__tauPanelBestC2={
    ...
    const __copyCandidateFromOtherPanel=()=>{
        const src=(window as any).__tauPanelBestC3;
        if(!src) return false;
    ...
    const __gotoRankedCandidate=(rank:number)=>{
        ...
        console.log("C2 rank fallback -> bestTex", {rank,tau:bestTau,subseq:bestSubseq,mode:__getRankMode()});
        return __requestTauRecompute(true);
    };
    ...
    canvas.addEventListener("click",(ev:any)=>{
        try{
            ...
            __lastSimKey="";
            (window as any).simReqRebuild=(((window as any).simReqRebuild|0)+1);
            console.log("C2 click override a(t)", {tau:tSel,subseq:sSel,aOverride:__aOverrideA,ratioY:ry});
        }catch{}
    });
    ...
    const __initTauControls=()=>{
        ...
        __tauCtlBestBtn.textContent="Moverse C2";
        ...
        __tauCtlRankBtn.textContent="Ir candidato C2";
        ...
        __tauCtlCopyBtn.textContent="Moverse al candidato de C3";
        ...
        let inserted=false;
        const h2=document.querySelector("#F2") as HTMLElement|null;
        ...
        if(!inserted){
            (__c2Host||document.body).appendChild(__tauCtlRoot);
        }
...
*/
    ...
    const __publishSimTrajectoryFromTau=(force:boolean=false)=>{
        ...
        if(typeof datosX1==="undefined" || !datosX1) return false;
        ...
        const src=(datosX1 instanceof Float32Array)?datosX1:new Float32Array(datosX1);
        ...
        for(let i=0;i<n;i++){
            if(!Number.isFinite(simX[i])) simX[i]=__safeNum(src[i],0);
        }

        (window as any).simDataX=simX;
        (window as any).simSamplesX=n;
        (window as any).chromatinIndex=chromSel;
        (window as any).simUseMeanSubseq=useMeanMode;
        (window as any).simMetaX={tau:tauSel,subseq:subSel,chromatin:chromSel,n,meanAllSubseq:useMeanMode,meanCnt,axis:"x"};
        (window as any).simStampX=((((window as any).simStampX)|0)+1);
        __lastSimKey=simKey;

        if(__lastSimLogFrame>0){
            console.log("C2 simX->C1", {tau:tauSel,subseq:subSel,chromatin:chromSel,mode:(useMeanMode?"mean-subseq":"single-subseq"),meanCnt,aOverride:(__aOverrideEnabled?__aOverrideA:null),samples:n,gain,realStepMean,modelStepMean,stampX:(window as any).simStampX});
            __lastSimLogFrame-=1;
        }
        return true;
    };
...
*/
    ...
    const __ensureTauHud=()=>{
        ...
        tauHudCanvas.id="tauHudOverlayC2";
        ...
        const p=(__c2Host||document.body) as any;
    ...
    const __syncTauHud=()=>{
        ...
        const isFs=(document.fullscreenElement===canvas || document.fullscreenElement===__fsTargetC2);
        if(isFs){
    ...
    const __publishMSDProgress=()=>{
        const prog=(window as any).__tauMSDProgressX;
        const fill=document.getElementById("c2-msd-progress-fill") as HTMLElement|null;
        const label=document.getElementById("c2-msd-progress-label") as HTMLElement|null;
        ...
        if(label){
            ...
            label.textContent="MSD SSE C2: "+phase+" ("+pct.toFixed(1)+"%) tau="+tauTxt+" subseq="+subTxt;
        }
    };
    // Mezcla una actualizacion parcial del progreso con el estado previo publicado.
    const __setMSDProgress=(patch:any)=>{
        const prev=(window as any).__tauMSDProgressX||{axis:"x",phase:"en espera",percent:0,tau:null,subseq:null,done:0,total:0,complete:false};
        const next=Object.assign({},prev,patch||{});
        (window as any).__tauMSDProgressX=next;
    ...
    const __updateMSDProgressTick=()=>{
        if((window as any).__tauMSDProgressX) __publishMSDProgress();
    };
    ...
    const __buildMSDScoreAxisJob=()=>{
        ...
        const srcBase=(typeof datosX1!=="undefined" && datosX1)?(datosX1 instanceof Float32Array?datosX1:new Float32Array(datosX1)):null;
        const tauMaxN=Math.max(1,((typeof tauMaxVeces!=="undefined"?tauMaxVeces:100)|0));
        ...
        const key=["x",modelStamp,tauMinN,tauMaxN,chromSel,srcBase?.length||0,((typeof tauMSDMaxLag!=="undefined"?tauMSDMaxLag:64)|0)].join("|");
        ...
        (window as any).__tauMSDScoreAxisX={raw,valid,width:tauMaxN,height:tauMaxN,stamp:modelStamp,complete:false,cursor:0,total:candidates.length};
    ...
    const __stepMSDScoreAxisJob=()=>{
        ...
        (window as any).__tauMSDScoreAxisX={raw:job.raw,valid:job.valid,width:job.tauMaxN,height:job.tauMaxN,stamp:job.modelStamp,complete,cursor:job.cursor,total:job.total};
    ...
    const __getMSDAxisPack=()=>{
        if(__tauMSDJob) return (window as any).__tauMSDScoreAxisX||null;
        if(__tauMSDLocalRaw && __tauMSDLocalValid) return (window as any).__tauMSDScoreAxisX||null;
        return null;
    };
    ...
    const __refreshMSDScoreDisplay=()=>{
        ...
        const other=(window as any).__tauMSDScoreAxisY;
    ...
    const __drawTauHud=()=>{
        ...
        const isFsNow=(document.fullscreenElement===__fsTargetC2);
        ...
        if(__dashMode){
            const srcX=(typeof datosX1!=="undefined" && datosX1)?(datosX1 instanceof Float32Array?datosX1:new Float32Array(datosX1)):null;
            const simX=((window as any).simDataX)?(((window as any).simDataX instanceof Float32Array)?(window as any).simDataX:new Float32Array((window as any).simDataX)):null;
            ...
            if(srcX && simX){
                const maxTau=Math.max(8,Math.min(220,Math.min(srcX.length,simX.length)-1));
                const msdO=calcMSD1D(srcX,maxTau);
                const msdS=calcMSD1D(simX,maxTau);
                ...
                hud.font=Math.max(6,Math.round(H2*0.0076)+1-notFsDelta)+"px monospace";
                hud.fillText("MSD eje x: orig blanca, sim 1D verde", xL+10, yTop0+38);
                hud.restore();
            }
        }
    ...
    const __fsTargetC2=__c2Host as any;
    const __fsHostIsCanvas=(__fsTargetC2===canvas);
    const __baseHostStyle=__fsHostIsCanvas?null:{
        position: (__c2Host.style.position || ""),
        top: (__c2Host.style.top || ""),
        left: (__c2Host.style.left || ""),
        width: (__c2Host.style.width || ""),
        height: (__c2Host.style.height || ""),
        maxWidth: (__c2Host.style.maxWidth || ""),
        maxHeight: (__c2Host.style.maxHeight || ""),
        zIndex: (__c2Host.style.zIndex || ""),
        display: (__c2Host.style.display || ""),
        margin: (__c2Host.style.margin || ""),
        padding: (__c2Host.style.padding || "")
    };
    ...
    const __applyCanvasFullscreenC2=()=>{
        const isFs=(document.fullscreenElement===__fsTargetC2);
        if(isFs){
            document.body.style.overflow="hidden";
            document.documentElement.style.overflow="hidden";
            if(!__fsHostIsCanvas){
                __c2Host.style.position="fixed";
                __c2Host.style.left="0";
                __c2Host.style.top="0";
                __c2Host.style.width="100vw";
                __c2Host.style.height="100vh";
                __c2Host.style.maxWidth="100vw";
                __c2Host.style.maxHeight="100vh";
                __c2Host.style.zIndex="999998";
                __c2Host.style.display="block";
                __c2Host.style.margin="0";
                __c2Host.style.padding="0";
            }
            ...
            if(!__fsHostIsCanvas && __baseHostStyle){
                __c2Host.style.position=__baseHostStyle.position;
                __c2Host.style.top=__baseHostStyle.top;
                __c2Host.style.left=__baseHostStyle.left;
                __c2Host.style.width=__baseHostStyle.width;
                __c2Host.style.height=__baseHostStyle.height;
                __c2Host.style.maxWidth=__baseHostStyle.maxWidth;
                __c2Host.style.maxHeight=__baseHostStyle.maxHeight;
                __c2Host.style.zIndex=__baseHostStyle.zIndex;
                __c2Host.style.display=__baseHostStyle.display;
                __c2Host.style.margin=__baseHostStyle.margin;
                __c2Host.style.padding=__baseHostStyle.padding;
            }
    ...
    canvas.addEventListener("dblclick", ()=>{
        ...
        else openFullscreen(__fsTargetC2);
        setTimeout(__applyCanvasFullscreenC2, 0);
    });
    document.addEventListener("fullscreenchange", ()=>{ __applyCanvasFullscreenC2(); __syncTauHud(); });
    window.addEventListener("resize", ()=>{ __applyCanvasFullscreenC2(); __syncTauHud(); });

    console.log("[C2 boot] cuerpo inicial completado");
