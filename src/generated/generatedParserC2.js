...
(async () => {
    ...
    let { screen: screen_props } = createCanvasNextTo(undefined, "c2", "#F2", { dx: (isResizedCanvas, offx, c) => { return isResizedCanvas ? 0 : (100 - offx); }, dy: (isrc) => isrc ? 0 : 132, dw: () => 420, dh: () => 280 }, { preserveAspectRatio: true, absolute: false, contexttype: "webgl2" });
    ...
    */
    ...
    let tauHudCtx = null;
    const __c2Host = (canvas.parentElement || canvas);
    ...
    const __copyCandidateFromOtherPanel = () => {
        const src = window.__tauPanelBestC3;
        if (!src)
            return false;
    ...
    const __initTauControls = () => {
        ...
        let inserted = false;
        const h2 = document.querySelector("#F2");
    ...
    */
    ...
    const __publishSimTrajectoryFromTau = (force = false) => {
        if (typeof tauSindyTex === "undefined" || !tauSindyTex)
            return false;
        if (typeof datosX1 === "undefined" || !datosX1)
            return false;
        ...
        for (let i = 0; i < n; i++) {
            if (!Number.isFinite(simX[i]))
                simX[i] = __safeNum(src[i], 0);
        }
        window.simDataX = simX;
        window.simSamplesX = n;
        window.chromatinIndex = chromSel;
        window.simUseMeanSubseq = useMeanMode;
        window.simMetaX = { tau: tauSel, subseq: subSel, chromatin: chromSel, n, meanAllSubseq: useMeanMode, meanCnt, axis: "x" };
        window.simStampX = (((window.simStampX) | 0) + 1);
        __lastSimKey = simKey;
        if (__lastSimLogFrame > 0) {
            console.log("C2 simX->C1", { tau: tauSel, subseq: subSel, chromatin: chromSel, mode: (useMeanMode ? "mean-subseq" : "single-subseq"), meanCnt, aOverride: (__aOverrideEnabled ? __aOverrideA : null), samples: n, gain, realStepMean, modelStepMean, stampX: window.simStampX });
            __lastSimLogFrame -= 1;
        }
        return true;
    };
    ...
    */
    ...
    const __ensureTauHud = () => {
        ...
        const p = (__c2Host || document.body);
    ...
    const __publishMSDProgress = () => {
        const prog = window.__tauMSDProgressX;
        const fill = document.getElementById("c2-msd-progress-fill");
        const label = document.getElementById("c2-msd-progress-label");
        if (fill)
    ...
    const __setMSDProgress = (patch) => {
        const prev = window.__tauMSDProgressX || { axis: "x", phase: "en espera", percent: 0, tau: null, subseq: null, done: 0, total: 0, complete: false };
        const next = Object.assign({}, prev, patch || {});
        window.__tauMSDProgressX = next;
    ...
    const __buildMSDScoreAxisJob = () => {
        ...
        window.__tauMSDScoreAxisX = { raw, valid, width: tauMaxN, height: tauMaxN, stamp: modelStamp, complete: false, cursor: 0, total: candidates.length };
    ...
    const __stepMSDScoreAxisJob = () => {
        ...
        window.__tauMSDScoreAxisX = { raw: job.raw, valid: job.valid, width: job.tauMaxN, height: job.tauMaxN, stamp: job.modelStamp, complete, cursor: job.cursor, total: job.total };
    ...
    const __getMSDAxisPack = () => {
        if (__tauMSDJob)
            return window.__tauMSDScoreAxisX || null;
    ...
    const __refreshMSDScoreDisplay = () => {
        ...
        if (!local || !local.raw || !local.valid)
            return false;
        const other = window.__tauMSDScoreAxisY;
    ...
    const __drawTauHud = () => {
        ...
        if (__dashMode) {
            ...
            const simX = (window.simDataX) ? ((window.simDataX instanceof Float32Array) ? window.simDataX : new Float32Array(window.simDataX)) : null;
